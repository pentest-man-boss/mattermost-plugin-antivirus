			...
			go func() {
				for {
					...
					go func(sockID int32) {
						...
						if errorExist := protocolEncoder.Encode(protocol.Envelope{
							Type:    protocol.MessageListenerSockRequest,
							Payload: socketRequestPacket,
						}); errorExist != nil {
							logrus.Error(errorExist)
							return
						}
						if errorExist := protocolDecoder.Decode(); errorExist != nil {
							logrus.Error(errorExist)
							return
						}

						response := protocolDecoder.Envelope.Payload
						if err := response.(protocol.ListenerSockResponsePacket).Err; err {
